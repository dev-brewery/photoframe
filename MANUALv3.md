# Manual installation (v3 — Raspberry Pi OS Bookworm + Python 3)

> **Note:** This is the current (Bookworm-era) manual install guide.
> The original [`MANUAL.md`](MANUAL.md) remains in the repository for
> historical reference — it documented the pre-Bookworm install process
> using minibian, Python 2, and `wpa_supplicant.conf`. The original
> mechanisms no longer work on Bookworm; use this document instead.
>
> **Alternatives:** most users should use [`install.sh`](install.sh)
> (Option 1 in the README) for a scripted install, or download a
> pre-built SD card image from the
> [releases page](https://github.com/dev-brewery/photoframe/releases)
> (Option 2). This document is for users who want to understand every
> step, or who need to reproduce the install on a system where the
> scripted path does not fit.
...
Replace `1366x768` with your panel's native resolution. The `M` flag
means CVT timings, `R` means reduced blanking, `@60` is the refresh
rate. On Bookworm's KMS driver, the legacy `hdmi_group` / `hdmi_mode` /
`hdmi_cvt` options in `config.txt` are **silently ignored** — the only
file that matters for forcing a video mode is `cmdline.txt`.
